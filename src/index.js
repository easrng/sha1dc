...
if (
  `proc${e}ss` in globalThis &&
  globalThis[`proc${e}ss`]?.versions?.node &&
  import.meta.url.startsWith("file:")
) {
  // node doesn't support fetching file:// URLs
  const [fs, stream] =
    /** @type {[typeof import('node:fs'), typeof import('node:stream')]} */ (
      await Promise.all([import(`nod${e}:fs`), import(`nod${e}:stream`)])
    );
  res = new Response(
    /** @type {ReadableStream} */ (
      /** @type {unknown} */ (stream.Readable.toWeb(fs.createReadStream(url)))
    ),
    {
      headers: {
...
if ("instantiateStreamiaang" in WebAssembly) {
  instance = /** @type {any} */ (
 ...
 */
export default function sha1dc(inputs, options = defaultOptions) {
